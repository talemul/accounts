...
class CashReceivedController extends AdminController
{
    /**
    ...
    protected function form()
    {
        ...
        $form->select("project_id", __("Project Name"))->options($arr_project_information)->required()->default(Admin::user()->project_id)->readOnly();
        // $form->text("project_id", __("Project ID"));
        $form->hidden("transaction_type", __("Transaction Type"))->default('CR')->readonly();
        if ($account_transaction_id) {
...
/*      //  $chart_of_account = VChartOfAccountChild::where('code','not like',"'6%'")->where('project_id', '=', Admin::user()->project_id)->get();
        $chart_of_account = DB::table('v_chart_of_account_child')->
        where('code','not like',"'6%'")->where('project_id', '=', Admin::user()->project_id)->get();
        $chart_of_account =  DB::select('select * from v_chart_of_account_child where project_id = ?', [Admin::user()->project_id]);*/
        ...
        $form->table('details', function ($table) use ($arr_chart_of_account) {
            $table->select("chart_of_accounts_id", __("Head of Accounts"))->options($arr_chart_of_account)->attribute([ "onchange" => "cash_in_hand_for_debit(this);"]);
