...
class ChartOfAccountController extends Controller
{
    ...
    {
        $grid = new Grid(new ChartOfAccount);
       ...
       // $grid->code_tech_desc('code_tech_desc');
        ...
        $grid->id('Create Action')->display(function ($id) use ($grid) {
            return '<a style="max-width:100px; overflow:auto;word-wrap: break-word;" href="/admin/chart_of_accounts/create?p=' . $this->id . '&l=' . (((int)$this->acc_level) + 1) . '&c=' . $this->code . '&pi=' . $this->project_id. '" target="_self"> Add</a>';
        });
        $grid->model()->orderBy('code', 'asc');
        $grid->model()->orderBy('project_id', 'asc');
        $grid->quickSearch(function ($model, $query) {
            $model->where('code', $query)->orWhere('code_desc', 'like', "%{$query}%");
        });
...
//            $actions->prepend('<a href="/chart_of_accounts/create?p=' . $pk . '><i class="fa fa-eye"></i> create new</a>');
    ...
    {
        $show = new Show(ChartOfAccount::findOrFail($id));
    ...
    {
        $form = new Form(new ChartOfAccount);
        $p_id = 0;
        $acc_level = 1;
        $code = '0';
        $product_id = '0';
        if (request()->has('p')) {
            ...
            $siblings_count = ChartOfAccount::where('p_id', '=', $p_id)->where('project_id', '=', Admin::user()->project_id)->get()->count();
            if ($siblings_count) {
        ...
        if ($idCheck) {
            $chart_of_account = ChartOfAccount::find($idCheck);
            ...
            if (request()->has('p')) {
                ...
                $code = ChartOfAccount::where('p_id', '=', 0)->where('project_id', '=', Admin::user()->project_id)->get()->count() + 1;
                if ($code == 0) {
                    $code = 1;
                }
            }
        ...
        $form->saved(function (Form $form) {
            ...
            if ($chart_of_account->p_id == 0) {
                ...
                $p_chart_of_account = ChartOfAccount::find($chart_of_account->p_id);
