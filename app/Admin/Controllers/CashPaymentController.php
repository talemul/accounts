...
class CashPaymentController extends AdminController
{
    /**
    ...
    protected function form()
    {
        ...
        $form->hidden("transaction_type", __("Transaction Type"))->default('CP')->readonly();
        if ($account_transaction_id) {
...
/*        $chart_of_account = VChartOfAccountChild::where('code','not like','6%')->get();
        // dd($chart_of_account);
        ...
        $form->table('details', function ($table) use ($arr_chart_of_account) {
            $table->select("chart_of_accounts_id", __("Head of Accounts"))->options($arr_chart_of_account)->attribute([ "onchange" => "cash_in_hand_for_credit(this);"]);
