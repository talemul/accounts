...
class AccountTransactionController extends AdminController
{
    ...
    protected function form()
    {
        ...
        $form->hidden("transaction_type", __("Transaction Type"))->default('JV')->readonly();
        if ($account_transaction_id) {
...
// Subtable fields
        ...
        /*     $chart_of_account = VChartOfAccountChild::get();
            // dd($chart_of_account);
        ...
        foreach ($chart_of_account as $key => $value) {
            if ($value->id != 0) {
                ...
                } else if ($value->project_id == Admin::user()->project_id && substr($value->code, 0, 1) == '5') {
                } else {
                    $arr_chart_of_account[$value->id] = $value->origin;
                }
            }
        ...
        $form->table('details', function ($table) use ($arr_chart_of_account) {
            $table->select("chart_of_accounts_id", __("Head of Accounts"))->options($arr_chart_of_account);
        ...
        $form->saved(function (Form $form) {
            ...
            $account_transactioninfo->voucher_no = AccountTransaction::where('transaction_type', '=', 'JV')->where('project_id', '=', $account_transactioninfo->project_id)->count();
