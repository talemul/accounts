...
class BankPaymentController extends AdminController
{
    /**
    ...
    protected function form()
    {
        ...
        $form->hidden("transaction_type", __("Transaction Type"))->default('BP')->readonly();
        if ($account_transaction_id) {
...
// Subtable fields
      ...
      /*  $chart_of_account = VChartOfAccountChild::where('code','not like','5%')->get();
        ...
        foreach ($chart_of_account as $key => $value) {
            if ($value->id != 0){
                if($value->project_id==Admin::user()->project_id && substr($value->code, 0, 1)!='5'){
                    $arr_chart_of_account[$value->id] = $value->origin;
                }
            }
        }
        $form->table('details', function ($table) use ($arr_chart_of_account) {
            $table->select("chart_of_accounts_id", __("Head of Accounts"))->options($arr_chart_of_account)->attribute([ "onchange" => "cash_at_bank_for_credit(this);"]);
